...
def limit_size(dataset, size):
    indexes = list(np.random.choice(range(0, dataset.dataset.__len__()), size=size, replace=False).astype(int))
    dataset.dataset.targets = np.array(dataset.dataset.targets)[indexes]
